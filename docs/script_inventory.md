...
| `analysis/comparison_analysis_paper.rmd` | Consolidated paper comparison analysis | `results/tables/*_circ.csv`, `results/tables/*_enrichment.csv`, chondrocyte/cartilage Shiny RDS files | Manuscript figures and supplementary tables in `results/publication_figures/` | None | Run via `--scripts=paper`; the target first processes all mouse datasets required by the focal and pairwise manuscript analyses, while omitting the baboon atlas |
